  ...
  return (
    <div className="space-y-3">
      ...
      {admins.map(admin => {
        ...
        return (
          <div key={admin.id} className="border border-[#1a1a1a] rounded-2xl bg-[#0a0a0a] overflow-hidden transition-all hover:border-[#2a2a2a]">
            {/* Row header */}
            <div
              className="flex items-center gap-4 px-5 py-4 cursor-pointer"
              onClick={() => setExpandedId(isOpen ? null : admin.id)}
            >
              <div className="w-9 h-9 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400 font-bold text-sm flex-shrink-0">
                {admin.email[0]?.toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-white truncate">{admin.email}</p>
                <p className="text-xs text-gray-600 mt-0.5">Joined {new Date(admin.created_at).toLocaleDateString()}</p>
              </div>
            ...
            {/* Expanded panel */}
            {isOpen && (
              <div className="border-t border-[#1a1a1a] px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Quota editing */}
                <div>
                ...
                {/* Feedback */}
                {feedback && (
                  <div className={`md:col-span-2 flex items-center gap-2 text-xs rounded-lg px-3 py-2 border ${feedback.ok ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
